...
Let's do that now. Go ahead and use `rownames_to_columns()` on `mtcars`, then make use of that `%>%` to pipe the result to `arrange`, sorting by **mpg** descending order. Finally, pipe the result to a `head` as we only want to gander at the first few rows:
...
```{r tidy-sort-chain-check, exercise=TRUE, exercise.lines = 3}
grade_result(

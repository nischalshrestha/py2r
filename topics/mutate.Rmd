...
To create new columns, you might be used to created it by assigning a new 'key' to the DataFrame using `[`:
...
or you can use `assign` in Python passing in keyword arguments:
...
- Then, modify let's continue the metric system conversion by dropping **mpg** column and instead creating a new **kpl** which is kilometer per liter. Here's the conversion: `mpg * 0.425144`.

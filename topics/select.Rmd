...
Let's take a brief look at reordering columns of a dataframe. Let's suppose we want to put the **carb** all the way at the front followed by **cyl** then finally **mpg** and the rest of the columns.
...
In R, you could do it in base-R, but this makes it quite confusing and suffers the same issue as Python in that you can to keep track of what the index was for each column.
...
**GOTCHA!** Using `-` to drop columns using labels is also not possible in base R:
